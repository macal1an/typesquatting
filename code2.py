...
# Список известных доменов (используем только их для анализа)
known_domains = list(domain_popularity.keys()) # Используем ключи словаря

# Извлекаем данные и фильтруем только известные домены
domain_values = []
for line in data.strip().split('\n'):
    domain, value = extract_domain_and_value(line)
    if domain and domain in known_domains and value is not None:  # Фильтруем и проверяем value
...
# Получаем значения популярности из словаря
popularities = []
for domain in domains:
    popularities.append(domain_popularity[domain])

# Вычисляем коэффициент корреляции Пирсона
correlation, p_value = pearsonr(popularities, values)

print(f"Коэффициент корреляции Пирсона между популярностью и степенью подделываемости: {correlation}")
print(f"P-значение: {p_value}")

# Диаграмма рассеяния
plt.scatter(popularities, values)
plt.xlabel("Рейтинг популярности (меньше = популярнее)")
plt.ylabel("Степень подделываемости")
plt.title("Корреляция между популярностью и степенью подделываемости")
plt.show()
